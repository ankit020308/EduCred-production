...
export default function InsightCard({ title, value, icon: Icon, trend, color = 'blue' }) {
  ...
  return (
    ...
    >
      <div className={`absolute top-0 right-0 w-32 h-32 blur-[50px] pointer-events-none group-hover:opacity-20 transition-all duration-700 ${color === 'blue' ? 'bg-blue-600/5' : 'bg-blue-600/5'}`} />
      ...
      <div className="space-y-6 relative z-10 mb-8">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10">
            <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${color === 'blue' ? 'bg-blue-500' : 'bg-blue-500'}`} />
      ...
      <div className="pt-6 mt-auto flex items-center justify-between relative z-10 border-t border-white/5">
        <div className="flex items-center gap-3">
            <TrendingUp size={14} className={color === 'rose' ? 'text-rose-500' : 'text-blue-500'} />
            <span className="small-text font-bold text-slate-400 uppercase tracking-widest text-[9px]">{trend || 'Optimal Output'}</span>
        </div>
      </div>

      <div className={`absolute bottom-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-blue-500/20 to-transparent group-hover:via-blue-500/40 transition-all opacity-0 group-hover:opacity-100 ${color === 'blue' ? 'via-blue-500/20' : ''}`} />
    </motion.div>
  );
}
