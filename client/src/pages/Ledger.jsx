...
const typeConfig = {
  ISSUE:   { icon: ShieldCheck, color: 'text-indigo-400',   bg: 'bg-indigo-400/10',   border: 'border-white/10' },
  VERIFY:  { icon: Search,      color: 'text-emerald-400', bg: 'bg-emerald-400/10', border: 'border-white/10' },
  REQUEST: { icon: Clock,       color: 'text-amber-400',   bg: 'bg-amber-400/10',   border: 'border-white/10' },
  APPROVE: { icon: UserCheck,   color: 'text-green-400',   bg: 'bg-green-400/10',   border: 'border-white/10' },
...
export default function Ledger() {
  ...
  return (
    <div className="relative min-h-screen bg-[#010409] text-slate-300 overflow-x-hidden selection:bg-indigo-500/30 font-sans">
      ...
      <div className="container max-w-5xl mx-auto px-6 pt-40 pb-24 relative z-10 space-y-16">
        ...
        <motion.div {...viewTransition} className="flex flex-col md:flex-row md:items-end justify-between gap-12">
          <div className="space-y-6">
            <div className="inline-flex items-center gap-3 px-4 py-1.5 rounded-full bg-white/[0.03] border border-white/5 backdrop-blur-md animate-levitate shadow-xl shadow-indigo-500/5">
              <Network className="text-indigo-400" size={16} />
              <span className="text-[9px] font-black text-indigo-400 uppercase tracking-[0.4em]">Global Protocol Consensus</span>
            </div>
            <h1 className="text-6xl md:text-[5.5rem] font-bold text-white tracking-tighter leading-none">
              Public <span className="bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text text-transparent text-glow-emissive">Ledger.</span>
            </h1>
          ...
          <div className="relative w-full md:w-80 group">
              <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-700 group-focus-within:text-indigo-500 transition-colors" size={18} />
              <input 
                placeholder="SEARCH TRANSMISSION..." 
                value={search} 
                onChange={e => setSearch(e.target.value)}
                className="w-full bg-white/[0.02] border border-white/10 rounded-2xl py-5 pl-14 pr-6 text-[10px] font-bold tracking-widest text-white outline-none focus:border-indigo-500/40 transition-all placeholder:text-slate-800 uppercase"
              />
          </div>
        </motion.div>
        ...
        <div className="relative">
          {/* Vertical axis line */}
          <div className="absolute left-10 top-0 bottom-0 w-px bg-gradient-to-b from-indigo-500/30 via-white/5 to-transparent hidden md:block" />
              ...
              <AnimatePresence>
                {filtered.length === 0 ? (
                  ...
                  return (
                    ...
                    >
                      {/* Interactive Ledger Node */}
                      <div className={`absolute left-0 top-1/2 -translate-y-1/2 w-20 h-20 rounded-[1.8rem] bg-[#010409] border hidden md:flex items-center justify-center z-10 shadow-3xl transition-transform hover:scale-110 duration-500 ${conf.border}`}>
                         <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${conf.bg} ${conf.color} shadow-inner`}>
                            <Icon size={24} />
                         </div>
                      </div>

                      <div className="glass-card p-10 border border-white/5 group relative hover:border-indigo-500/20 transition-all duration-700">
                         {/* Subtle glowing Orb behind event */}
                         ...
                         <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 relative z-10 mb-10 pb-8 border-b border-white/5">
                            ...
                            {ev.txHash && (
                              <div className="flex items-center gap-4 group/tx cursor-help">
                                 <Activity size={14} className="text-indigo-500/40 group-hover/tx:text-indigo-400 group-hover/tx:animate-pulse transition-colors" />
                                 <span className="text-[10px] font-mono text-indigo-500/20 group-hover/tx:text-indigo-400 transition-colors uppercase tracking-[0.2em] italic truncate max-w-[280px]">
                         ...
                         <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-10 relative z-10">
                            <div className="space-y-4">
                               ...
                               <div className="flex items-center gap-4 text-slate-500">
                                  ...
                                  <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">{ev.universityName || 'Network Hub'}</span>
                               </div>
                            </div>
